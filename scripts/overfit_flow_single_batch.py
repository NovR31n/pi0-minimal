"""Overfit one real LIBERO sample with a fixed conditional flow problem."""
...
def main() -> None:
    ...
    fixed_time = torch.full((1,), 0.5, dtype=torch.float32, device=args.device)
    ...
    history.append({"step": 0, "loss": initial_loss, "velocity_mae": initial_mae})
    ...
    final_mae = float(history[-1]["velocity_mae"])
    loss_ratio = final_loss / initial_loss
    accepted = loss_ratio <= args.acceptance_ratio and final_mae < initial_mae
    result = {
        "seed": args.seed,
        "steps": args.steps,
        ...
        "initial_velocity_mae": initial_mae,
        "final_velocity_mae": final_mae,
    ...
    if not accepted:
        raise RuntimeError(
            f"single-batch overfit failed: loss ratio {loss_ratio:.6f} "
...
def _evaluate(
    ...
    *,
    ...
    policy.eval()
    with _autocast_context("cuda" if use_autocast else "cpu"):
        output = policy.training_step(
            condition,
            state,
            expert_actions,
    ...
    policy.train()
    absolute_error = (
        output.predicted_velocity - output.flow_batch.target_velocity
    ).abs()
